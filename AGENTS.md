...
The app is a 3-service stack: Next.js frontend (root) → NestJS + Prisma API (`backend/`) → Postgres. The frontend talks to the API via `NEXT_PUBLIC_API_URL`; the admin data layer is [`src/lib/admin-context.tsx`](src/lib/admin-context.tsx) calling [`src/lib/api.ts`](src/lib/api.ts).
