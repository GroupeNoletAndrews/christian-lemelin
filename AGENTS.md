...
- Admin auth is **Supabase Auth** (email + password) via [`@supabase/ssr`](https://supabase.com/docs/guides/auth/server-side/nextjs) — the session rides in cookies and auto-refreshes. Clients: [`frontend/src/lib/supabase/client.ts`](frontend/src/lib/supabase/client.ts) (browser sign-in), [`frontend/src/lib/supabase/server.ts`](frontend/src/lib/supabase/server.ts) (route handlers / server components), [`frontend/src/lib/supabase/proxy.ts`](frontend/src/lib/supabase/proxy.ts) (session refresh). `requireAdmin()` in [`frontend/src/lib/server/auth.ts`](frontend/src/lib/server/auth.ts) validates the session (`supabase.auth.getUser()`) and is the real authorization boundary inside each protected handler; [`frontend/src/proxy.ts`](frontend/src/proxy.ts) (Next 16's renamed `middleware.ts`) refreshes the session on `/admin/*` and does the coarse `/admin/dashboard` redirect. Public sign-up is **disabled** (create users in Supabase Studio); optional `ADMIN_EMAILS` allowlist gates entry. Auth is the *only* thing on Supabase Auth — **all data access stays Prisma** (no PostgREST/RLS).
...
- Vercel only builds `frontend/` (set as the project's root directory); the repo root holds just config and docs.
