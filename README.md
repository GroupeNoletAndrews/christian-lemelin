...
Réalisations have an **admin-controlled display order** (drag-and-drop in the dashboard; persisted via `PATCH /api/realisations/reorder`). `/realisations` shows the same masonry grid as the home page; clicking any project opens a **full-screen viewer** ([`RealisationLightbox`](frontend/src/components/realisations/RealisationLightbox.tsx)). `?featured=<id>` still works as a deep link — it opens the viewer on that project.
