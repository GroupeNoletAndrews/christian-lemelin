...
| 2026-06-03 | **Page Réalisations + grille uniforme + modale de candidature.** Cartes réalisations extraites dans `RealisationCard` (ratio uniforme `aspect-[4/3]`) ; la section d'accueil passe d'un masonry à colonnes à une **grille régulière** (toutes les tuiles à la même taille). Nouvelle page publique **`/realisations`** (`src/app/(app)/realisations/page.tsx`) : hero OPUS + grille de toutes les réalisations (carrousel au survol) + CTA sombre `ink`. Page **`/emplois`** : le bouton « Postuler » ouvre désormais une **modale de candidature** (`ApplyModal` — nom, courriel, téléphone, message, CV optionnel, état de succès ; Échap/clic-fond pour fermer, scroll-lock). Pas de back-end pour les candidatures reçues (UI seulement). |
